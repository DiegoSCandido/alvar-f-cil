...
export function AlvaraForm({
  ...
  const [requestDateInput, setRequestDateInput] = useState(() =>
    formatDateForInput(editingAlvara?.requestDate || new Date()),
  );
  const [expirationDateInput, setExpirationDateInput] = useState(() =>
    formatDateForInput(editingAlvara?.expirationDate),
  );
  ...
  useEffect(() => {
    if (open) {
      ...
      if (editingAlvara) {
        ...
        setRequestDateInput(formatDateForInput(editingAlvara.requestDate));
        ...
        setRequestDateInput(formatDateForInput(new Date()));
        setExpirationDateInput("");
        setAnoSelecionado(anoAtual);
      }
    } else {
      ...
      <>
        ...
        <div className="space-y-3 border rounded-md p-3 sm:p-4 mt-0 bg-gray-50 shadow-sm">
          <div className="flex items-center gap-2 mb-2">
            <Label htmlFor="ano-taxas" className="text-sm font-medium">
              Ano:
            </Label>
            <Input
              id="ano-taxas"
          ...
          <div className="flex flex-col gap-3">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              ...
              <Input
                type="date"
                value={
                  taxaAno.dataTaxaEnviada
                    ? formatDateForInput(taxaAno.dataTaxaEnviada)
                    : ""
                }
                onChange={(e) =>
                  setTaxaAno(anoSelecionado, {
                    dataTaxaEnviada: e.target.value
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3">
              <div className="flex items-center gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setTaxaAno(anoSelecionado, { taxaPaga: e.target.checked })
                  }
                  className="accent-o2-blue h-4 w-4 transition-colors duration-150 border-gray-300 rounded focus:ring-2 focus:ring-o2-blue"
                />
                <Label
              ...
              <Input
                type="date"
                value={
                  taxaAno.dataTaxaPaga
                    ? formatDateForInput(taxaAno.dataTaxaPaga)
                    : ""
                }
                onChange={(e) =>
                  setTaxaAno(anoSelecionado, {
                    dataTaxaPaga: e.target.value
                      ? new Date(e.target.value)
                      : null,
                  })
                }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Erro ao salvar alvará";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleConfirmRenewalFinalized = async () => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Erro ao finalizar renovação";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleRenovationUpdated = async (e: React.FormEvent) => {
    ...
    try {
        ...
        try {
          for (const file of renewalFiles) {
            ...
            formDataUpload.append(
              "nomeDocumento",
              `Documento Renovação - ${file.name}`,
            );
            formDataUpload.append("tipoDocumento", "renovacao");
            const token = localStorage.getItem("authToken");
            const response = await fetch(
              `${import.meta.env.VITE_API_URL || "/api"}/documentos-alvara/upload/${editingAlvara.id}`,
              {
                method: "POST",
                body: formDataUpload,
                headers: token
            ...
            if (!response.ok) {
              ...
              throw new Error(
                error.error || "Erro ao fazer upload do documento",
              );
            }
          ...
          setError(
            "Erro ao fazer upload de alguns documentos. As observações foram salvas.",
          );
      ...
      const message =
        err instanceof Error ? err.message : "Erro ao atualizar renovação";
      setError(message);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleViewDocument = async (fileName: string) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/documentos-alvara/alvara/${editingAlvara.id}`,
        {
      ...
      let doc = docs.find((d: any) => {
        ...
        return (
          normalizedDocName === normalizedFileName ||
      ...
      if (!doc) {
        const renovacaoDocs = docs.filter(
          (d: any) => d.tipo_documento === "renovacao",
        );
      ...
      const res2 = await fetch(
        `${API_BASE_URL}/documentos-alvara/download/${doc.id}`,
        {
          headers: token ? { Authorization: `Bearer ${token}` } : undefined,
        },
      );
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent
          className={`w-[95vw] sm:w-full p-3 sm:p-4 lg:p-6 ${isRenewing ? "sm:max-w-[600px] lg:max-w-[700px] xl:max-w-[800px]" : "sm:max-w-[500px] lg:max-w-[600px] xl:max-w-[700px]"} max-h-[90vh] overflow-y-auto`}
        >
          <DialogHeader>
            <DialogTitle>
              {editingAlvara
          ...
          <form
            ...
            className={`${isRenewing ? "space-y-3" : "space-y-4"}`}
          >
            ...
            <div className="space-y-2">
              ...
              >
                <SelectTrigger
                  id="clienteId"
                  className={`text-sm ${isAlvaraEmAbertura || isRenewing ? "bg-muted cursor-not-allowed" : ""}`}
                >
            ...
            <div className="space-y-2">
              ...
              {editingAlvara ? (
                ...
                <Select
                  value={formData.type}
                  onValueChange={(value) =>
                    setFormData({ ...formData, type: value as AlvaraType })
                  }
                  required
                ...
                >
                  <SelectTrigger
                    id="type"
                    className={`text-sm ${isRenewing ? "bg-muted cursor-not-allowed" : ""}`}
                  >
                    <SelectValue
                      placeholder={
                        formData.clienteId
                  ...
                  <SelectContent>
                    ...
                    {formData.type &&
                      (!tiposPermitidos.includes(formData.type) ||
                        tiposPermitidos.length === 0) && (
              ...
              {isRenewing && (
                <p className="text-xs text-muted-foreground">
                  O tipo de alvará não pode ser alterado em processo de
                  renovação.
                </p>
              )}
            </div>
            ...
            <div className="space-y-2">
              <Label htmlFor="requestDate" className="text-xs sm:text-sm">
                Data Solicitação *
              </Label>
              <Input
                id="requestDate"
                ...
                value={requestDateInput}
                onChange={(e) => {
                  const inputValue = e.target.value;
                  setRequestDateInput(inputValue);
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Textarea
                  ...
                  placeholder={
                    isRenewing
                      ? "Digite uma nota sobre a renovação..."
                  ...
                  <div className="bg-gray-50 rounded p-2 text-xs space-y-0 overflow-y-auto max-h-32 border border-gray-200 min-h-fit">
                    {formData.notes
                      ...
                      .map((note, idx) => {
                        ...
                        const fileNames = isDocNote
                          ? extractFileNames(note)
                          : [];
                        return (
                          <div
                            key={idx}
                            className="flex items-start gap-2 text-xs py-1.5 px-1 border-b border-gray-200 last:border-b-0"
                          >
                            <span className="text-amber-600 shrink-0 mt-0.5 font-bold">
                              •
                            </span>
                            ...
                            {isDocNote &&
                              ...
                              editingAlvara?.id && (
                                <button
                                  type="button"
                                  onClick={() =>
                                    handleViewDocument(fileNames[0])
                                  }
                ...
                {renewalFiles.length > 0 && (
                  <div className="space-y-1 mt-2">
                    ...
                    {renewalFiles.map((file, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between gap-2 p-2 bg-muted rounded text-xs border"
                      >
            ...
            <DialogFooter
              className={`gap-2 flex flex-col-reverse ${isRenewing ? "sm:flex-row sm:justify-end" : "sm:flex-row"}`}
            >
              <Button
                type="button"
                variant="outline"
      ...
      <Dialog open={isConfirmingRenewal} onOpenChange={setIsConfirmingRenewal}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid w-full items-center gap-2 mt-2">
            ...
            />
            {finalizeFile && (
              <span className="text-xs text-o2-blue">{finalizeFile.name}</span>
            )}
            {uploading && (
              <span className="text-xs text-o2-blue">
                Enviando documento...
              </span>
            )}
          </div>
