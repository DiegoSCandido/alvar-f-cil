...
import { ATIVIDADES_SECUNDARIAS_LIST } from '@/lib/atividades-secundarias-data';
...
interface AtividadeSecundariaSelectProps {
  label: string;
  codigoValue: string;
...
export function AtividadeSecundariaSelect({
  label,
  codigoValue,
  descricaoValue,
  onSelect,
}: AtividadeSecundariaSelectProps) {
  const [open, setOpen] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [filtradas, setFiltradas] = useState<AtividadeOption[]>([]);
  ...
  const handleSearch = (query: string) => {
    ...
    if (query.length === 0) {
      setFiltradas(ATIVIDADES_SECUNDARIAS_LIST.slice(0, 50)); // Mostra primeiras 50
    } else {
  ...
  const handleOpenChange = (newOpen: boolean) => {
    ...
    if (newOpen) {
      setSearchQuery('');
      setFiltradas(ATIVIDADES_SECUNDARIAS_LIST.slice(0, 50)); // Mostra primeiras 50 ao abrir
  ...
  return (
    <div className="space-y-2">
      <Label htmlFor="atividade-select">{label}</Label>
